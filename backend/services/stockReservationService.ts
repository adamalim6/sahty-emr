...
class StockReservationService {
    ...
    async hold(tenantId: string, req: HoldRequest): Promise<ReservationLine> {
        return tenantTransaction(tenantId, async (client: PoolClient) => {
            ...
            // 2. Lock current_stock row FOR UPDATE
            ...
            `, [lineId, reservationId, tenantId, req.stock_demand_line_id || null,
                req.product_id, req.lot, req.expiry, req.source_location_id, 
                req.destination_location_id || null, req.qty_units]);

            // 4. Update reserved_units (delta: +qty)
            await client.query(`
     ...
     * 
     * Moves stock, zeros reserved_units, marks COMMITTED.
    ...
    async commitSession(
        ...
        return tenantTransaction(tenantId, async (client: PoolClient) => {
            ...
            const headerResult = await client.query(`
                SELECT * FROM stock_reservations 
                WHERE tenant_id = $1 AND session_id = $2 AND status = 'ACTIVE' AND expires_at > NOW()
                FOR UPDATE
            `, [tenantId, sessionId]);
            ...
            if (lines.rows.length === 0) return null;

            // 3. Fetch demand for destination
            ...
            const headerDestId = serviceLoc.rows.length > 0 ? serviceLoc.rows[0].id : 'UNKNOWN';
            const sourceLocId = lines.rows[0].source_location_id;

            // 4. Create transfer header (with stock_reservation_id for lineage)
            ...
            `, [transferId, tenantId, demandId, sourceLocId, headerDestId, userId, header.reservation_id]);

            // 5. Process each line
            for (const line of lines.rows) {
                const take = line.qty_units;
                const destId = line.destination_location_id || headerDestId;

                // 5a. Lock and decrement source stock
                ...
                if (decResult.rowCount === 0) {
                    throw new Error(`STALE_RESERVATION: Insufficient stock for product ${line.product_id}, lot ${line.lot}`);
                }
                ...
                // 5d. Create transfer line (with reservation_line_id for traceability)
                ...
                `, [transferLineId, tenantId, transferId, line.product_id, line.lot, line.expiry, 
                    take, line.source_location_id, destId, line.stock_demand_line_id || null, line.id]);
            }

            // 6. Mark header COMMITTED (keep lines for audit)
            await client.query(`
                UPDATE stock_reservations 
