...
export const LabTaxonomyService = {
     ...
     * but strictly joins via the tenant's localized lab_section_tree mapping.
     */
    async getSectionsBySousFamille(tenantId: string, sousFamilleId: string, client?: PoolClient) {
        return db.withTenantClient(tenantId, async (tenantClient) => {
            ...
            const query = `
                ...
                JOIN lab_section_tree t ON t.section_id = s.id
     ...
     * Joins flat definitions via the localized lab_sub_section_tree mapping.
     */
    async getSubSectionsBySection(tenantId: string, sectionId: string, client?: PoolClient) {
        return db.withTenantClient(tenantId, async (tenantClient) => {
            ...
            const query = `
                ...
                JOIN lab_sub_section_tree t ON t.sub_section_id = ss.id
