...
async function runTenantMigrations() {
    console.log('--- Running Tenant Migrations 083 (Remove lab_units) ---');
    ...
    for (const clientRow of clients) {
        ...
        try {
            ...
            const sqlPath = path.join(__dirname, '../migrations/pg/tenant/083_remove_lab_units_tenant.sql');
            let sql = fs.readFileSync(sqlPath, 'utf8');
            console.log(`  Executing 083_remove_lab_units_tenant.sql...`);
