...
const OUT_FILE = '/Users/adamalim/Desktop/current_units.csv';
...
async function main() {
  ...
  try {
    ...
    let res;
    try {
        res = await client.query('SELECT * FROM public.units ORDER BY created_at ASC');
    } catch {
        res = await client.query('SELECT * FROM reference.units ORDER BY created_at ASC');
    }
    
    if (res.rows.length === 0) {
      console.log('No data found in units table.');
      return;
    }
