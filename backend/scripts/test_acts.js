...
const { LabReferenceRepository } = require('../repositories/labReferenceRepository');
...
async function run() {
  ...
  try {
    ...
    if (res.rows.length > 0) {
      ...
      `, [sampleId]);
      console.log('Resolved analytes for act:', res2.rows);

      // Testing exact query from repository
      ...
      `, [[sampleId]]);
      console.log('Filtered exact repository analytes:', res3.rows);
    }
  } catch (e) {
    console.error(e);
  } finally {
