...
async function runTenantMigrations() {
    console.log('--- Running Tenant Migration 114 (HPRIM Integration) ---');
    ...
    for (const clientRow of clients) {
        ...
        try {
            ...
            const sqlPath = path.join(__dirname, '../migrations/pg/tenant/114_hprim_integration.sql');
            let sql = fs.readFileSync(sqlPath, 'utf8');
            console.log(`  Executing 114_hprim_integration.sql...`);
