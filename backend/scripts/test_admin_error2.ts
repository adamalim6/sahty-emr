...
async function run() {
    ...
    // Get a valid user
    const uRes = await pool.query(`SELECT user_id FROM auth.users LIMIT 1`);
    const userId = uRes.rows.length > 0 ? uRes.rows[0].user_id : '00000000-0000-0000-0000-000000000000';

    console.log("Testing with prescription event:", eventId, " and user:", userId);
