...
-- Seed basic types (optional, but good for validity)
...
-- 7. Drop Legacy Tables (CAUTION: Ensure code usage is removed first or this will break things)
-- We will comment this out for now and run it manually or in a separate step after code refactor?
-- User instructions said "Drop legacy global identity tables after new ones created".
-- I'll include it but maybe we should ensure safety.
-- Let's execute it as the user explicitly asked for "SQL migrations... drop legacy...".

DROP TABLE IF EXISTS public.global_identity_documents CASCADE;
DROP TABLE IF EXISTS public.patients_global CASCADE;
