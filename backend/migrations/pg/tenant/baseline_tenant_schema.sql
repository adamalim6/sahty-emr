...
CREATE TABLE IF NOT EXISTS public.pricing_list_organismes (
    id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pricing_list_id             UUID NOT NULL REFERENCES public.pricing_lists(id) ON DELETE CASCADE,
    organisme_id                UUID NOT NULL REFERENCES reference.organismes(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS public.pricing_list_items (
    id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pricing_list_id             UUID NOT NULL REFERENCES public.pricing_lists(id) ON DELETE CASCADE,
    global_act_id               UUID NOT NULL REFERENCES reference.global_actes(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS public.pricing_list_item_versions (
    id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pricing_list_item_id        UUID NOT NULL REFERENCES public.pricing_list_items(id) ON DELETE CASCADE,
    version_no                  INTEGER NOT NULL,
    status                      TEXT NOT NULL DEFAULT 'DRAFT'
                                    CHECK (status IN ('DRAFT', 'PUBLISHED', 'ARCHIVED')),
    supersedes_version_id       UUID NULL REFERENCES public.pricing_list_item_versions(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS public.pricing_list_item_version_dispatches (
    id                              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pricing_list_item_version_id    UUID NOT NULL REFERENCES public.pricing_list_item_versions(id) ON DELETE CASCADE,
    ...
    allocation_value                NUMERIC(12,2) NOT NULL CHECK (allocation_value >= 0),
    created_at                      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
