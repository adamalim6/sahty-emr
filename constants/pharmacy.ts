...
import { InventoryItem, ItemCategory, ProductDefinition, ProductType, StockLocation, PartnerInstitution, StockOutTransaction, StockOutType, DestructionReason } from '../types/pharmacy';

const MOCK_LOCATION_NAMES = ['Étagère A-1', 'Étagère A-2', 'Réfrigérateur 1', 'Armoire Sécurisée', 'Réserve Vrac B'];

export const generateMockLocations = (): StockLocation[] => {
  return MOCK_LOCATION_NAMES.map((name, index) => ({
    id: `LOC-${100 + index}`,
    name: name,
...
export const generateMockPartners = (): PartnerInstitution[] => {
  return [
    {
      id: 'PART-001',
...
export const generateMockCatalog = (): ProductDefinition[] => {
  return [
    {
      id: 'PROD-001',
      name: 'Amoxicilline 500mg Gélules',
...
export const generateMockStockOutHistory = (): StockOutTransaction[] => {
    return [
        {
            id: 'SORT-2025-001',
