...
export const FamillesPage: React.FC = () => {
    const [familles, setFamilles] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState('');
    
    // Modal State
    const [selectedFamille, setSelectedFamille] = useState<any | null>(null);
    ...
    const loadFamilles = async () => {
        setLoading(true);
        try {
            const response = await api.getFamilles();
            setFamilles(response);
        } catch (error) {
            console.error('Failed to load familles:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreate = () => {
        setSelectedFamille({ code: '', libelle: '', actif: true });
        setIsModalOpen(true);
    };

    const handleEdit = (famille: any) => {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await api.deleteFamille(id);
            loadFamilles();
        } catch (error) {
            console.error('Failed to delete famille:', error);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (selectedFamille.id) {
                await api.updateFamille(selectedFamille.id, selectedFamille);
            } else {
                await api.createFamille(selectedFamille);
            }
            setIsModalOpen(false);
            loadFamilles();
        } catch (error) {
            console.error('Failed to save famille:', error);
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Familles d'Actes</h1>
                    <p className="text-slate-500">Gestion de la taxonomie principale (Niveau 1)</p>
                </div>
                <button 
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                filteredFamilles.map((famille) => (
                                    <tr key={famille.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${famille.actif ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
            ...
            {isModalOpen && selectedFamille && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl flex flex-col overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50">
                            <div>
                                <h2 className="text-xl font-bold text-slate-800">
                                    {selectedFamille.id ? 'Modifier la Famille' : 'Nouvelle Famille'}
                                </h2>
                            </div>
                            <button 
                        ...
                        <form id="famille-form" onSubmit={handleSave} className="p-6 space-y-5">
                            <div>
                                ...
                                <input 
                                    ...
                                    value={selectedFamille.code}
                                    onChange={e => setSelectedFamille({...selectedFamille, code: e.target.value})}
                                />
                            </div>
                            <div>
                                ...
                                <input 
                                    ...
                                    value={selectedFamille.libelle}
                                    onChange={e => setSelectedFamille({...selectedFamille, libelle: e.target.value})}
                                />
                            </div>
                            <div className="flex items-center pt-2">
                                <label className="flex items-center space-x-3 cursor-pointer">
                                    <input 
                                        ...
                                        checked={selectedFamille.actif !== false}
                                        onChange={e => setSelectedFamille({...selectedFamille, actif: e.target.checked})}
                                    />
                                    <span className="text-slate-700 font-medium">Famille Active</span>
                                </label>
                            </div>
                        </form>
