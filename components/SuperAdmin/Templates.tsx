...
import { FileText, Plus, Search, Edit2, Copy, Save, ShieldCheck, Compass } from 'lucide-react';
...
export const TemplatesManager: React.FC = () => {
    ...
    const fetchPhrases = async () => {
        setIsLoading(true);
        try {
            const data = await api.getGlobalSmartPhrases();
            setPhrases(data);
        } catch (error: any) {
    ...
    const handleCreateNew = () => {
        ...
        setFormData({ trigger: '', label: '', description: '', body_html: '' });
        setIsEditing(true);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            let savedPhrase;
            if (isNew) {
                savedPhrase = await api.createGlobalSmartPhrase(formData);
                toast.success('Smart Phrase created!');
            } else {
                savedPhrase = await api.updateGlobalSmartPhrase(selectedPhrase.id, formData);
                toast.success('Smart Phrase updated!');
            }

            fetchPhrases();
            setIsEditing(false);
            
            // Re-select the saved phrase
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 max-w-[1600px] mx-auto w-full">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                            ...
                            Global Smart Phrases
                        </h1>
                        <p className="text-slate-500 mt-1">Manage system-wide templates replicated to all tenants.</p>
                    </div>
                    <button 
                        ...
                        {isLoading ? (
                            ...
                            <div className="space-y-1">
                                {filteredPhrases.map(phrase => (
                                    ...
                                    >
                                        <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${phrase.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                            ...
                            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
                                <div>
                                    <h2 className="font-semibold text-slate-900">
                                        {!selectedPhrase ? 'Nouvelle Global Phrase' : (isEditing ? 'Édition de la phrase' : 'Détails de la phrase')}
                                    </h2>
                            ...
                            <div className="flex-1 overflow-y-auto p-6">
                                <div className="max-w-2xl space-y-6">
                                    ...
                                    <div>
                                        ...
                                        />
                                        <div className="flex items-start gap-2 mt-3 p-3 bg-indigo-50 border border-indigo-100 rounded-lg">
