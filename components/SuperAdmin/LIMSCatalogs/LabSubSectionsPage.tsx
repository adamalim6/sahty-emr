...
export const LabSubSectionsPage = () => {
    return (
        <LimsCatalogGrid 
            resource="lab-sub-sections"
            title="Sous-Sections Laboratoire"
            columns={[]}
            renderForm={(initialData, onSubmit, onCancel, isSaving, error) => {
                return <LabSubSectionForm initialData={initialData} onSubmit={onSubmit} onCancel={onCancel} isSaving={isSaving} error={error} />;
            }}
        />
    );
};

const LabSubSectionForm = ({ initialData, onSubmit, onCancel, isSaving, error }: any) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Code Sous-Section</label>
                <input required type="text" value={code} onChange={e => setCode(e.target.value.toUpperCase())} className="w-full border-slate-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 uppercase font-mono text-sm" placeholder="Ex: BIOCH_SANG" />
            </div>

            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Libellé</label>
                <input required type="text" value={libelle} onChange={e => setLibelle(e.target.value)} className="w-full border-slate-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Ex: Biochimie Sanguine" />
            </div>
