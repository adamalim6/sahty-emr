...
import { ProductDefinition, ProductType, Supplier, Molecule, DosageUnit } from '../../types/pharmacy';
import { 
...
export const ProductCatalog: React.FC<ProductCatalogProps> = ({ products, onAddProduct, onUpdateProduct }) => {
  ...
  const getActivePrice = (suppliers: Supplier[] = []): number => {
  ...
  const unitPriceTTC = calculateUnitPriceTTC(priceTTC, formData.subdivisionUnits || 1);
                    ...
                    <div className="border-t border-slate-100 my-2 pt-2">
                       ...
                       {product.isSubdivisable && (
                         <div className="flex justify-between items-center mt-1">
                            <span className="text-xs text-slate-400">Prix Unitaire ({product.subdivisionUnits} unités)</span>
                            <span className="font-mono text-xs text-slate-600">
                              €{calculateUnitPriceTTC(pTTC, product.subdivisionUnits || 1).toFixed(3)} /unité
                            </span>
                         </div>
                       )}
                    </div>
           ...
           <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="space-y-3">
                 ...
                 {formData.suppliers?.map((supplier, idx) => (
                    <div key={supplier.id} className={`flex items-center space-x-3 p-3 rounded-lg border ${supplier.isActive ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200'}`}>
                       <input 
                         type="radio" 
                       ...
                       <input 
                         ...
                         className="flex-1 bg-white border border-slate-300 rounded px-2 py-1 text-sm text-black outline-none focus:border-blue-400 placeholder-slate-400"
                       />
              ...
              {formData.isSubdivisable && (
                 <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                    <div>
                       ...
                       <input 
                         type="number" 
                         value={formData.subdivisionUnits || ''}
                         onChange={(e) => setFormData({...formData, subdivisionUnits: parseInt(e.target.value)})}
