...
export const SuggestionList = forwardRef((props: SuggestionListProps, ref) => {
  ...
  useEffect(() => {
      // Reset selected index when the search query changes the items
      setSelectedIndex(0);
  }, [props.items]);
        ...
        {props.items.map((item, index) => (
          <button
            className={`w-full text-left px-3 py-2 flex flex-col focus:outline-none rounded-md transition-colors mb-0.5 ${
              index === selectedIndex 
                ? 'bg-sahty-50 text-sahty-900 border border-sahty-200 shadow-sm outline-none' 
          ...
          >
            ...
            <span className={`text-xs truncate ${index === selectedIndex ? 'text-sahty-700' : 'text-gray-500'}`}>
                {item.label || 'No description'}
            </span>
          </button>
        ))}
      </div>
