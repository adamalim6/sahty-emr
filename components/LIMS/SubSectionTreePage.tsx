...
import { 
    useLimsSubSectionTree, 
    useLimsConfigDictionaries, 
    useCreateSubSectionTree, 
    useUpdateSubSectionTree,
    useSetSubSectionTreeStatus 
} from './useLimsConfig';
import { Plus, CheckCircle2, XCircle, Search, Save, X, Edit2, Check } from 'lucide-react';
import { SearchableSelect } from '../ui/SearchableSelect';

export const SubSectionTreePage = () => {
    const { data: treeList = [], isLoading: isTreeLoading } = useLimsSubSectionTree();
    const { sections, subSections, isLoading: isDictLoading } = useLimsConfigDictionaries();
    const createMutation = useCreateSubSectionTree();
    const updateMutation = useUpdateSubSectionTree();
    const statusMutation = useSetSubSectionTreeStatus();
    ...
    const filteredTree = treeList.filter(t => 
        (t.sub_section_label || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const subSectionOptions = subSections.map(ss => ({ value: ss.id, label: `${ss.libelle} [${ss.code}]`, searchString: `${ss.libelle} ${ss.code}` }));

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.section_id || !formData.sub_section_id) return;
        
        await createMutation.mutateAsync(formData);
        setIsModalOpen(false);
        setFormData({ section_id: '', sub_section_id: '', sort_order: 0, actif: true });
    };
    ...
    const saveEdit = async (e?: React.MouseEvent) => {
        if (e) e.stopPropagation();
        if (!editData || !editData.section_id || !editData.sub_section_id) return;
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50/50">
            ...
            <div className="bg-white px-8 py-5 border-b border-slate-200 flex justify-between items-center shrink-0 shadow-sm z-10">
                <div>
                    <h1 className="text-xl font-bold text-slate-800">Sous-chapitres</h1>
                    <p className="text-sm text-slate-500 mt-1">Assigner des sous-chapitres aux chapitres principaux</p>
                </div>
                <button
                ...
                >
                    <Plus size={18} />
                    <span>Nouveau Sous-chapitre</span>
                </button>
            </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                        <div className="relative w-96">
                            ...
                            <input
                                type="text"
                                placeholder="Rechercher par chapitre ou sous-chapitre..."
                                value={searchQuery}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                                    ...
                                    <th className="px-6 py-4">Sous-chapitre</th>
                                                ...
                                                {isEditing ? (
                                                    <div onClick={e => e.stopPropagation()}>
                                                        <SearchableSelect
                                                            ...
                                                            value={editData.sub_section_id}
                                                            onChange={val => setEditData({ ...editData, sub_section_id: val })}
                                                            placeholder="Sélectionner un sous-chapitre..."
                                                        />
                                                    </div>
                                                ) : (
                                                    <>
                                                        <div className="font-medium text-slate-800">{item.sub_section_label}</div>
                                                        <div className="text-xs text-slate-500">[{item.sub_section_code}]</div>
                                                    </>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                {isEditing ? (
                                                    <div className="flex items-center justify-end space-x-2">
                                                        ...
                                                        <button 
                                                            onClick={saveEdit}
                                                            disabled={updateMutation.isPending || !editData.section_id || !editData.sub_section_id}
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h3 className="font-bold text-slate-800 text-lg">Nouveau Sous-chapitre</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Chapitre (Section) *</label>
                                <select
                                    required
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Sous-chapitre LIMS *</label>
                                <select
                                    required
                                ...
                                >
                                    ...
                                    {subSections.map(ss => (
                                        <option key={ss.id} value={ss.id}>{ss.libelle} [{ss.code}]</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-100 flex justify-end space-x-3">
                                ...
                                <button
                                    type="submit"
                                    disabled={!formData.section_id || !formData.sub_section_id || createMutation.isPending}
