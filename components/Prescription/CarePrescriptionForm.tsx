...
interface CarePrescriptionFormProps {
...
export const CarePrescriptionForm: React.FC<CarePrescriptionFormProps> = ({ onSave }) => {
    ...
    const getValidationState = () => {
        ...
        if (selectedActs.length === 0) {
            errorMessages.push("Veuillez sélectionner au moins un acte ou soin.");
            newFieldErrors['acts'] = true;
        }
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-5">
                                ...
                                {Array.from({ length: getDaysInMonth(pickerDate.getFullYear(), pickerDate.getMonth()) }).map((_, i) => {
                                    ...
                                    return (
                                        <button type="button" key={day} onClick={() => handleDateClick(day)} disabled={isDisabled}
                                            className={`h-9 w-9 rounded-full text-sm font-medium flex items-center justify-center transition-all ${isSelected ? 'bg-orange-600 text-white shadow-md' : isDisabled ? 'text-slate-300 cursor-not-allowed bg-slate-50' : 'text-slate-700 hover:bg-slate-100'}`}>
                                            {day}
                                        </button>
                                    );
                                })}
                            ...
                            <div className="bg-slate-50 p-3 rounded-xl mb-6 border border-slate-100">
                                ...
                                <div className="relative">
                                    ...
                                    <select value={tempTime} onChange={(e) => setTempTime(e.target.value)}
                                        className="w-full pl-9 pr-10 py-2 h-12 bg-white border border-slate-200 rounded-lg appearance-none focus:ring-2 focus:ring-orange-500 outline-none font-bold text-slate-800 text-lg cursor-pointer">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button type="button" onClick={confirmDateTime} disabled={!tempTime} className={`flex-1 py-2.5 text-sm font-semibold text-white rounded-xl shadow-lg transition ${!tempTime ? 'bg-slate-300' : 'bg-orange-600 hover:bg-orange-700'}`}>Valider</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="bg-orange-600 px-6 py-4 flex items-center gap-3">
                        ...
                        <h2 className="text-lg font-semibold text-white">Actes & Soins Infirmiers</h2>
                    </div>
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                                Rechercher des actes ou soins
                            </label>
                            <div className="relative group">
                                <input
                                    type="text"
                                    placeholder="Ex: Pansement, Injection, Perfusion..."
                                    className={`w-full pl-10 pr-4 py-3 h-12 bg-slate-50 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all font-medium text-slate-800 placeholder:text-slate-400 ${fieldErrors['acts'] ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-200'}`}
                                    value={searchTerm}
                                    onChange={(e) => {
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <textarea
                                    id="comment"
                                    rows={3}
                                    placeholder="Ajouter des précisions pour le soin..."
                                    className="w-full px-4 py-3 pl-10 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all font-medium text-slate-800 placeholder:text-slate-400 resize-y"
                                    value={comment}
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="min-h-[160px]">
                            {prescriptionType === 'one-time' ? (
                                <div className="flex flex-col items-center justify-center text-center p-8 bg-orange-50/50 rounded-xl border border-orange-100 animate-in fade-in slide-in-from-right-2 duration-300">
                                    <div className="bg-white p-4 rounded-full shadow-sm mb-4">
                                        <Calendar className="w-8 h-8 text-orange-600" />
                                    </div>
                                    <h3 className="text-orange-900 font-bold text-lg mb-2">Acte Ponctuel</h3>
                                    <p className="text-slate-600 max-w-md leading-relaxed">
                                        L'acte ou soin prescrit sera réalisé une seule fois à la date et heure spécifiée ci dessous.
                                    </p>
                                    ...
                                    <div className="mt-8 w-full max-w-xs">
                                        ...
                                        <button
                                            ...
                                            className={`w-full bg-white border hover:border-orange-300 hover:ring-4 hover:ring-orange-50 transition-all rounded-xl px-4 py-3 flex items-center gap-3 group shadow-sm text-left ${fieldErrors['startDate'] ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-200'}`}
                                        >
                                            <div className="bg-orange-50 group-hover:bg-orange-100 p-2 rounded-lg transition-colors">
                                ...
                                // FULL FREQUENCY UI (Adapted from Imagery/Biology)
                                <div className="space-y-6 animate-in fade-in slide-in-from-left-2 duration-300">

                                    {/* IMMEDIATE DOSE BLOCK */}
                                    {prescriptionType === 'punctual-frequency' && (
                                        <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 flex items-center gap-4 shadow-sm">
                                            <div className="bg-white p-2 rounded-full border border-orange-100 shadow-sm">
                                                <Zap className="w-5 h-5 text-orange-600 fill-orange-100" />
                                            </div>
                                            <div className="flex-1">
                                                <h4 className="text-sm font-bold text-orange-900 flex items-center gap-2">
                                                    Soin Immédiat
                                                    <span className="bg-orange-200 text-orange-800 text-[10px] uppercase px-1.5 py-0.5 rounded font-bold tracking-wide">Automatique</span>
                                                </h4>
                                                <p className="text-xs text-orange-700 mt-0.5">Un soin unique sera généré pour <strong>maintenant</strong>.</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex gap-6">
                                        <label className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${scheduleData.dailySchedule === 'everyday' ? 'border-orange-600' : 'border-slate-300 group-hover:border-orange-400'}`}>
                                                {scheduleData.dailySchedule === 'everyday' && <div className="w-2.5 h-2.5 bg-orange-600 rounded-full" />}
                                            </div>
                                            <span className={`text-sm font-medium ${scheduleData.dailySchedule === 'everyday' ? 'text-orange-900' : 'text-slate-600'}`}>Tous les jours</span>
                                        ...
                                        <label className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${scheduleData.dailySchedule === 'every-other-day' ? 'border-orange-600' : 'border-slate-300 group-hover:border-orange-400'}`}>
                                                {scheduleData.dailySchedule === 'every-other-day' && <div className="w-2.5 h-2.5 bg-orange-600 rounded-full" />}
                                            </div>
                                            <span className={`text-sm font-medium ${scheduleData.dailySchedule === 'every-other-day' ? 'text-orange-900' : 'text-slate-600'}`}>1 jour / 2</span>
                                        ...
                                        <label className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${scheduleData.dailySchedule === 'specific-days' ? 'border-orange-600' : 'border-slate-300 group-hover:border-orange-400'}`}>
                                                {scheduleData.dailySchedule === 'specific-days' && <div className="w-2.5 h-2.5 bg-orange-600 rounded-full" />}
                                            </div>
                                            <span className={`text-sm font-medium ${scheduleData.dailySchedule === 'specific-days' ? 'text-orange-900' : 'text-slate-600'}`}>Jours spécifiques</span>
                                                ...
                                                {DAYS.map((day) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`flex-1 py-3 text-sm font-bold uppercase tracking-wider rounded-lg border-2 transition-all active:scale-95 ${isSelected ? 'border-orange-600 bg-orange-50 text-orange-600 shadow-inner' : 'border-orange-100 bg-white text-orange-300 hover:border-orange-300'}`}
                                                        >
                                                            {day}
                                                        </button>
                                                    )
                                    ...
                                    <div className="bg-orange-50/50 p-5 rounded-xl border border-orange-100/50">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-xs font-bold text-orange-900 uppercase tracking-widest">Mode de Fréquence</span>
                                            <div className="flex bg-white rounded-lg border border-orange-100 p-1 shadow-sm">
                                        ...
                                        {scheduleData.mode === 'cycle' ? (
                                            <div className="animate-in fade-in slide-in-from-bottom-2">
                                                <label className="block text-sm font-medium text-orange-900 mb-2">Répéter toutes les :</label>
                                                    ...
                                                    {['2', '4', '6', '8', '12', '24'].map((hr) => (
                                                        <button
                                                            ...
                                                            className={`flex-1 min-w-[60px] py-2 text-sm font-medium border rounded-lg transition-all active:scale-95 ${!scheduleData.isCustomInterval && scheduleData.interval === hr
                                                                ...
                                                                : 'bg-white text-slate-600 border-orange-200 hover:border-orange-400 hover:bg-orange-50'
                                                                }`}
                                                        >
                                                            {hr}h
                                                        </button>
                                                ...
                                                {scheduleData.isCustomInterval && (
                                                    <div className="mt-3 flex items-center gap-2 animate-in fade-in slide-in-from-top-1">
                                                        <input
                                                            ...
                                                            className={`w-24 p-2 text-sm text-center font-bold text-orange-800 bg-white border rounded-lg focus:ring-2 focus:ring-orange-500 outline-none ${fieldErrors['cycleInterval'] ? 'border-red-500 ring-2 ring-red-100' : 'border-orange-300'}`}
                                                            autoFocus
                                                        />
                                                        <span className="text-sm font-medium text-orange-800">heures</span>
                                                    </div>
                                                )}
                                            </div>
                                        ) : scheduleData.mode === 'simple' ? (
                                            <div className="animate-in fade-in slide-in-from-bottom-2">
                                                <label className="block text-sm font-medium text-orange-900 mb-2">Fréquence :</label>
                                                <div className="flex items-center gap-3">
                                                    <input
                                                        ...
                                                        className="w-20 p-2 text-center text-sm font-bold text-orange-800 bg-white border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none"
                                                    />
                                                    <span className="text-sm font-semibold text-orange-700">fois par jours</span>
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <div className="relative">
                                                                    <input
                                                                        ...
                                                                        className={`w-16 pl-2 pr-1 py-2 text-center font-bold text-slate-700 bg-white border rounded-lg focus:ring-2 focus:ring-orange-500 outline-none ${parseInt(scheduleData.simpleCount || '0') <= 1 ? 'opacity-50 cursor-not-allowed border-slate-200' :
                                            ...
                                            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`flex-1 py-2.5 px-3 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all active:scale-95 ${isSelected
                                                                    ...
                                                                    : 'bg-white border-orange-100 text-slate-600 hover:border-orange-300 hover:bg-orange-50'
                                                                    }`}
                                                            >
                                                                ...
                                                                <span className={`text-xs ${isSelected ? 'text-orange-200' : 'text-slate-400'}`}>{time}</span>
                                                            </button>
                                                        )
                                                    })}
                                                </div>
                                                ...
                                                <div>
                                                    <label className="block text-xs font-bold text-orange-900 uppercase tracking-wide mb-2">
                                                        Ajouter une heure personnalisée
                                                    </label>
                                                    <div className="relative">
                                                        <select
                                                            onChange={addCustomTime}
                                                            className="w-full pl-3 pr-10 py-2.5 h-12 bg-white border border-orange-200 rounded-xl appearance-none focus:ring-2 focus:ring-orange-500 outline-none text-sm font-medium text-slate-700 cursor-pointer hover:border-orange-400 transition-colors"
                                                            value=""
                                                        >
                                                        ...
                                                        <Plus className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-orange-400 pointer-events-none" />
                                                    </div>
                                                </div>
                                                ...
                                                {scheduleData.specificTimes && scheduleData.specificTimes.length > 0 && (
                                                    <div className="flex flex-wrap gap-2 pt-2">
                                                        {scheduleData.specificTimes.sort().map(time => (
                                                            <div
                                                                key={time}
                                                                className="group flex items-center gap-2 pl-3 pr-2 py-1.5 bg-white border border-orange-100 rounded-full shadow-sm text-orange-700 text-sm font-bold animate-in zoom-in-95"
                                                            >
                                                                <span>{time}</span>
                                                                <button
                                                                    type="button"
                                                                    onClick={() => toggleSpecificTime(time)}
                                                                    className="p-1 rounded-full hover:bg-red-50 text-orange-300 hover:text-red-500 transition-colors"
                                                                    title="Supprimer"
                                                                >
                                                                    <X className="w-3.5 h-3.5" />
                                                                </button>
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className={`w-full pl-3 pr-10 py-2.5 h-12 border rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm font-medium text-slate-700 [&::-webkit-calendar-picker-indicator]:hidden ${isSpecificTimeRestricted ? 'bg-slate-100 cursor-pointer' : 'bg-slate-50 border-slate-200'}`}
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={openCalendarModal}
                                                    className="absolute right-1 top-1 bottom-1 aspect-square bg-orange-600 hover:bg-orange-700 text-white rounded-lg flex items-center justify-center transition-colors shadow-sm z-10"
                                        ...
                                        <div>
                                            ...
                                            <div className={`flex shadow-sm rounded-xl overflow-hidden border h-12 ${fieldErrors['duration'] ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-200'}`}>
                                                <input
                                                    ...
                                                    min="1"
                                                    placeholder="--"
                                                    className="flex-1 text-center bg-white outline-none text-sm font-medium h-full selection:bg-orange-100 border-none focus:ring-0 p-2"
