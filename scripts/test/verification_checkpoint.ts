 ...
 * 
 * Usage: npx ts-node scripts/test/verification_checkpoint.ts [tenantId]
 */

import { globalQuery, globalQueryOne } from '../../backend/db/globalPg.js';
import { tenantQuery, getTenantPool } from '../../backend/db/tenantPg.js';
import { globalProductService } from '../../backend/services/globalProductService.js';
import { globalDCIService } from '../../backend/services/GlobalDCIService.js';
import { globalActesService } from '../../backend/services/globalActesService.js';
import { globalSupplierService } from '../../backend/services/globalSupplierService.js';
import { globalAdminService } from '../../backend/services/globalAdminService.js';
...
async function main() {
    ...
    results.push(await runTest('1.4 Count global_product_price_history', async () => {
        const row = await globalQueryOne<any>('SELECT COUNT(*) as c FROM global_product_price_history');
        return { count: parseInt(row?.c || '0') };
    }));

    results.push(await runTest('1.5 Count clients', async () => {
    ...
    results.push(await runTest('2.1 Raw tenantQuery SELECT 1', async () => {
        try {
            ...
            if (err.message.includes('does not exist')) {
                console.log(`   ⚠️ Tenant DB does not exist (expected for new tenants)`);
                return { note: 'Tenant DB does not exist - this is OK for new setups' };
            }
            throw err;
        }
    }));
    ...
    results.push(await runTest('3.7 globalAdminService.getAllGlobalRoles()', async () => {
